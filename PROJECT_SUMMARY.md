...
> هذا الملف يُحدَّث كل ما يتغير شيء جوهري في المشروع. للتفاصيل الكاملة، راجع `README.md`. لسجل التغييرات الزمني، راجع `CHANGELOG.md`. للمشاكل المعروفة، راجع `BUGS_AND_FIXES.md`.
...
- **جديد (2026-08-13):** نظام نشر تلقائي (`deploy/`) — أي commit يوصل لـ `main` على GitHub يتنزل ويتبني ويتشغّل تلقائيًا على الـ VM خلال دقيقة، مع rollback تلقائي عند الفشل. **معزول بالكامل عن الإنتاج** (بورتات/أسماء/صورة/volume منفصلة تمامًا — نفس مبدأ التجربة اليدوية على بورتات 18000/13000/15173/18787). التركيب لمرة واحدة عبر `deploy/install.sh`.
