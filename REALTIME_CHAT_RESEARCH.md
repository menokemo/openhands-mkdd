...
| F | إزالة الـ polling التراكمي | ✅ تم — الـ polling الرئيسي (كل ثانيتين) اتشال بالكامل؛ بقي resync خفيف كل 15 ثانية كـ fallback (تحديث الـ cost/execution status، والتعافي لو الـ WebSocket فشل يعيد الاتصال). **تم تأكيد محادثة حية فعلية على staging (2026-08-13): الردود توصل لحظيًا بدون refresh.** |
...
- **الاختبار الفعلي محدود ببيئة بدون Docker (من ناحيتي):** تم التحقق الكامل من: منطق الأمان (رفض/قبول الاتصال)، توافق HTTP+WS في نفس السيرفر، البناء والاختبارات. **تحديث — تم التأكيد الفعلي (2026-08-13):** المستخدم اختبر محادثة حقيقية كاملة على الـ staging وأكّد وصول الردود لحظيًا (live) بدون أي refresh يدوي. تم تشغيل النظام بنجاح من أول تجربة.
